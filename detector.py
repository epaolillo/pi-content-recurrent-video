...
def query_episodes_with_faiss(videos, vectors_dir):
    ...
    vector_files = [os.path.join(vectors_dir,e.rsplit('/')[-2:-1][0]+'.p') for e in videos]
    vectors = []
...
def detect(videos, feature_vector_function="CH", annotations=None, artifacts_dir=None, framejump=60, percentile=10,
    ...
    print("Comenzamos deteccion")
    print(f"Salto de frames: {framejump}")
    print(f"Tipo de feature vector: {feature_vector_function}")

    feature_vectors_dir_name = "vectors"

    # location of the vector directory
    vectors_dir = feature_vectors_dir_name
    ...
    for url in videos:        
        ...
        print("Converting {} to feature vectors".format(url))
        featurevectors.construct_feature_vectors(   
            url, feature_vectors_dir_name, feature_vector_function, framejump)
    ...
    for video, result in results:
        framerate = video_functions.get_framerate(video)
